...
export class AgentTunnel {
  ...
  private openRequest(id: number, method: string, path: string, headers: HeaderList): void {
    ...
    // Every HTML document a browser navigates to is edited on the way out:
    //  - its manifest link is made to fetch with credentials (behind the relay's
    //    login a cookie-less manifest fetch gets the login page instead);
    //  - when E2E is on, the E2E bootstrap is injected at the top of <head>, so
    //    fetch/WebSocket are patched before any of dsh's own scripts run (dsh
    //    opens its event socket and fires its first /api calls while plugin
    //    client modules are still loading — a shim delivered as a module
